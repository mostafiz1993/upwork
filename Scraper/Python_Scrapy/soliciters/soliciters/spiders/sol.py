...
class AuthorSpider(scrapy.Spider):
    ...
    def parse_author(self, response):
        ...
        yield {
            ...
            'phone_no' : response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[0].css('dd::text')[(response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[0].css('dt').extract().index('<dt>Tel:</dt>'))-2].extract() if '<dt>Tel:</dt>' in  response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[0].css('dt').extract() else '',
            'email' :  response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[0].css('dd.slidingDiv a::text').extract() if '<dt id="Email">Email: </dt>' in  response.css('div.accordion-group section').xpath("//div[@id='main-details-accordion']/dl")[0].css('dt').extract() else '',
            'website' : response.css('div.accordion-group section').xpath("//div[@id='main-details-accordion']/dl")[0].css('dd')[(response.css('div.accordion-group section').xpath("//div[@id='main-details-accordion']/dl")[0].css('dt').extract().index('<dt>Web:</dt>'))-1].css('a::text').extract() if '<dt>Web:</dt>' in  response.css('div.accordion-group section').xpath("//div[@id='main-details-accordion']/dl")[0].css('dt').extract() else '',
            'head/branch' : response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[1].css('em::text').extract_first()
            'address' : response.css('div.accordion-group section')[0].xpath("//div[@id='main-details-accordion']/dl")[1].css('dd.feature::text').extract()
            'people': parse_people_struct(response)
        }
    def parse_people_struct(self,response):
        for li in response.css('div.accordion-group section')[3].xpath("//div[@id='people-and-structure-accordion']/div")[0].css('ul li'):
            yield scrapy.Request(response.urljoin(li.css('a::attr(href)').extract()), callback=self.iterate_people)
        if response.css('div.search-results-controls div.pagination li.next a::attr(href)').extract():
            yield scrapy.Request(response.urljoin(response.css('div.search-results-controls div.pagination li.next a::attr(href)').extract_first()),callback=self.parse_people_struct)



    def iterate_people(self,response):
        for section in response.css('div.items section'):
            yield scrapy.Request(response.urljoin(section.css('header h2 a::attr(href)').extract_first(),callback=self.parse_people_detail))
    def parse_people_detail(self,response):
        yield{
            'person_name' : response.xpath("//div[@id='main-content']/article/header/h1/text()").extract_first(),
            'email' : response.css('div.panel-half dl')[0].css('dd.slidingDiv a::text').extract_first() if '<dt id="Email">Email: </dt>' in response.css('div.panel-half dl')[0].css('dt').extract() else '',
        }
